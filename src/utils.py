...
def resource_path(relative_path: str) -> str:
    ...
    try:
        ...
        # Check if running as a bundled executable (covers --onedir)
             ...
             # The executable's directory is the base path for --onedir bundles
             # Resources are typically placed relative to this executable
             base_path = os.path.dirname(sys.executable)
        else:
             # Not frozen, running from source (e.g., in PyCharm).
             # The base path is the directory containing *this* script file (__file__).
             # Use os.path.abspath to ensure it's an absolute path
             base_path = os.path.dirname(os.path.abspath(__file__))
             # If placing resources outside 'src' (e.g., in project root), adjust base_path:
             # base_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) # Go up one level from src

    except Exception as e:
         # Fallback in case of unforeseen errors (e.g., __file__ not defined)
    ...
    # Ensure the path uses correct separators for the OS, although os.path.join usually handles this
