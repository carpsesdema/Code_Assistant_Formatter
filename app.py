...
class SmartReplaceApp(QWidget):
    ...
    def _load_custom_font(self) -> QFont:
        ...
        try:
            # Use resource_path to find the font file (now expected in root)
            font_path_str = resource_path(FONT_FILENAME) # FONT_FILENAME = "JetBrainsMono-Regular.ttf"
    ...
    def _set_app_icon(self):
        ...
        try:
            # Use resource_path to find the icon file (expected in root)
            icon_path_str = resource_path(ICON_FILENAME) # ICON_FILENAME = "Code_Helper.ico"
    ...
    def format_new_code(self):
        ...
        try:
            # Use the utility function to format (includes pre-cleaning now)
    ...
    def apply_new_code(self):
        ...
        try:
            ...
            if format_error:
                # Ask user if they want to apply the *unformatted* (but potentially cleaned) code
                # Get the intermediate cleaned code before Ruff failed
                cleaned_but_not_ruffed, _ = format_code_with_ruff(new_code_raw) # Call again to get pre-Ruff state

                reply = QMessageBox.warning(self, "Formatting Error",
                                            f"Ruff failed to format the pasted code:\n{format_error}\n\nDo you want to apply the code *without full Ruff formatting*?",
                ...
                # User chose Yes, apply the pre-Ruff cleaned code
                final_code_to_write = cleaned_but_not_ruffed # Use the intermediate state
                log_color = COLOR_WARNING
                log_suffix = " (applied after pre-cleaning, but Ruff format failed)."
                self.log_message(f"Applying pre-cleaned code to {file_path.name} after Ruff formatting error.", COLOR_WARNING)
            else:
    ...
    def preview_diff(self):
        ...
        try:
            ...
            # 5. Format the pasted code using our utility
            new_code_formatted, format_error = format_code_with_ruff(new_code_raw)

            if format_error:
                # Warn but allow diffing against unformatted (but pre-cleaned) code
                cleaned_but_not_ruffed, _ = format_code_with_ruff(new_code_raw) # Get intermediate state

                reply = QMessageBox.warning(self, "Formatting Error",
                                            f"Could not format pasted code with Ruff:\n{format_error}\n\nShow diff against the *pre-cleaned* pasted code?",
                                            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
                                            QMessageBox.StandardButton.Yes)
                if reply == QMessageBox.StandardButton.No:
                    return # Abort diff
                # Proceed with pre-cleaned code
                new_lines = cleaned_but_not_ruffed.splitlines()
                to_desc = f"b/{file_path.name} (Pasted - Pre-cleaned)"
                self.log_message(f"Previewing diff against pre-cleaned pasted code due to Ruff error.", COLOR_WARNING)
            else:
                # Use fully formatted code
    ...
    def preview_snippet_change(self):
        ...
        try:
            ...
            if format_error:
                QMessageBox.warning(self, "Snippet Formatting Error", f"Could not format the pasted snippet (check syntax or Ruff issues):\n{format_error}")
                return # Stop if snippet itself is invalid or Ruff fails
            ...
            # Slicing needs exclusive end index, AST end_lineno is often the index *after* the last line in practice with ast.parse
            end_line_index = target_node.end_lineno
